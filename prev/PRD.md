...
| F4 | **Group** measurements and compute **aggregates**: total area, total length, point count, item count. | Groups as first-class concept with recomputation when inputs change. |
...
| F7 | Typed measurement kinds (e.g. polygon, polyline, rectangle, count) with correct formulae. | Ensures consistent semantics. |
...
3. Developer sets scale(s) per measurement or per group as needed.
...
|--------|------------------|
| **Accuracy** | Automated tests vs known baseline (reference lengths/areas for given pixels + scale). |
...
| **Scale attachment** | Open | Confirm whether scale is per-measurement, per-group, or both; current design supports association with measurements. |
...
*PRD version: 1.0 — reflects vision, v1 scope, and “no exported UI” for all releases.*
